...
>
  <div>
    ...
    <div class="mt-12 flex flex-col md:flex-row items-center gap-12 md:gap-24">
      ...
      <p
        class="md:mt-8 text-justify lg:text-xl max-w-[48ch] md:max-w-none border-t border-t-neutral-200 pt-8 md:pt-0 md:border-t-0 md:border-l md:border-l-neutral-200 md:pl-8"
      >
  ...
  <div class="mt-24 mb-16 xl:mb-0">
    ...
    <div class="relative mt-12">
      <p class="absolute -top-3 lg:-left-14 bg-neutral-50 pr-4">{cards[currentCard].year}</p>
      <hr class="bg-neutral-200 border-neutral-200 " />
    </div>
    <div class="mt-8 p-4 min-h-[9rem] border border-neutral-200 rounded-lg">
      <h4 class="font-semibold uppercase text-xl">
        {cards[currentCard].title}
      </h4>
      <p class="mt-2 lg:text-xl">
        {cards[currentCard].shortDesciprition}
      </p>
    </div>
    <div class="flex">
      <button
        class={`${currentCard === 0 ? 'hidden' : 'flex'} gap-2 uppercase mt-2 items-center`}
        on:click={() => (currentCard -= 1)}
      >
        <ArrowLeft /> Previous
      </button>
      <button
        class={`${currentCard === cards.length - 1 ? 'hidden' : 'flex'} ml-auto gap-2 uppercase mt-2 items-center`}
        on:click={() => (currentCard += 1)}
      >
        Next <ArrowRight />
      </button>
